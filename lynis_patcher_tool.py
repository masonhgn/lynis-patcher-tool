"""Tool that will automatically patch vulnerabilities pointed out by Lynis"""
#    ***********************************************************************************
...
import subprocess
from commands import suggestions_dict
...
def patch_suggestions(report: List[str]) -> None:
    """patches suggestions"""
    for line in report:
        if line in suggestions_dict:
            success = run_command(suggestions_dict[line])
            if success:
                print("Command \'", suggestions_dict[line], "\' succeeded")
            else:
                print("Command \'", suggestions_dict[line], "\' failed")
...
def main():
    ...
    file_path = input("""Please enter the exact path of your latest Lynis security vulnerability
 report. It should be located at /var/log/lynis-report.dat.....""")
    temp_report = parse_file(file_path)
    patch_suggestions(temp_report)
